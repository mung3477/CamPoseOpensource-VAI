...
# Force HF caches to a subdirectory under the repository root and offline-only behavior
...
def _make_prompt(dataset_path: str) -> str:
    ...
    if "can" in name:
        return "Move the can to the target"
    if "square" in name:
        return "Pick up the square nut and insert the peg into it"
    raise ValueError("Could not infer task prompt from dataset_path; expected one of ['lift','can','square'] in the name")
...
class SmolVLAPolicyWrapper(nn.Module):
    def __init__(self, args):
        ...
        # TEMP: Override optimizer hyperparameters from args
