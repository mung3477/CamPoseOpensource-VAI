...
class BackboneResNet(nn.Module):
    def __init__(self, hidden_dim: int, use_plucker: bool, imagenet: bool):
        ...
        # Create a SINGLE shared ResNet18 backbone for all cameras
        shared = torchvision.models.resnet18(
            replace_stride_with_dilation=[False, False, False],
            pretrained=imagenet, norm_layer=FrozenBatchNorm2d)

        if use_plucker:
            # Modify first conv layer to accept 9 channels (3 RGB + 6 Plucker)
        ...
        # ResNet18 has 512 output channels
        self.input_proj = nn.Conv2d(512, hidden_dim, kernel_size=1)
        # Max 2 cameras → 2 * 8 * 8 tokens
...
class BackboneLateConcat(nn.Module):
    def __init__(self, hidden_dim: int, use_plucker: bool, imagenet: bool, use_r3m: bool, latent_drop_prob: float = 0.0):
        ...
        # Shared RGB encoder (ResNet18 trunk up to C5)
        if use_r3m:
            # Load R3M weights for ResNet18 and keep the convolutional trunk up to C5
            # Requires the 'r3m' package to be installed and available in PYTHONPATH
            import r3m
            r3m_model = r3m.load_r3m("resnet18")
            # r3m exposes a torchvision ResNet under 'convnet'
        ...
        # Shared Plucker encoder: simple CNN that downsamples to 8x8 and outputs 512 channels
        if self.use_plucker:
...
class BackboneLinear(nn.Module):
    def __init__(self, camera_names: List[str], hidden_dim: int, patch_size: int, use_plucker: bool):
        super().__init__()
        ...
        if use_plucker:
            ...
            num_patches = len(camera_names) * (256 // patch_size) ** 2
            self.pos_embed = nn.Embedding(num_patches, hidden_dim)
